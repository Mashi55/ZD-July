    ...
    public partial class Form1 : Form
    {
        ...
        public void button2_Click(object sender, EventArgs e)
        {
            ...
            */
            ...
            if (SelectMode == 5) //wave functioncollapse
            {
            ...
            WaweCollapseEnd:
                ...
                /*
                for (int x = 0; x <= Cells[wawerandom - 8].EntropyPossible.Count-1; x++) //3x fut le
                {
                    label16.Text = Convert.ToString(TemporalStorage[x]);
                }
                ...
                */
                ...
                while (temporary < 5)
                {
                    ...
                    //kiválasztom a legkisebb entropy helyét random

                    int minimum = 5;

                    for (int x = 0; x <= 55; x++)
                    {
                        if (minimum < Cells[x].entropy)
                        {
                            minimum = Cells[x].entropy;
                        }
                    }

                    for (int x = 0; x <= 54; x++)
                    {
                        if (Cells[x].entropy == minimum)
                        {
                            valaszthato[x] = Cells[x].pos;
                        }
                    }
                    int randomValue = 0;

                    randomValue = valaszthato[random.Next(0, valaszthato.Length)];


                    minimum = 5;


                    current = randomValue;  //current --> ahova megyünk

                    //kiválasztom hogy milyet rakok oda

                    //pick = random.Next(0, 5);
                    pick = 0;
                    ...
                    //random kiválasztom hogy mit rakok oda be
                    ...
                    if (pick == 0)
                    {
                        ...
                        if (Cells[current].Neighbour[0] < 70)
                        {
                            for (int x = 0; x <= Cells[Cells[current].Neighbour[0]].possible.Length - 1; x++)   //annyiszor futhat, amennyi
                            {
                                hasonlitas = Cells[Cells[current].Neighbour[0]].possible[x]; //fenti szomszédja
                            ...
                            int[] Neww = NewPossible.ToArray();
                            Cells[Cells[current].Neighbour[0]].possible = Neww;
                            Cells[Cells[current].Neighbour[0]].entropy = Neww.Length;  //entropyt beallitjuk

                            Array.Clear(Neww, 0, Neww.Length); //clears array + list
                            NewPossible.Clear();
                        ...
                        if (Cells[current].Neighbour[1] < 70)
                        {
                            for (int x = 0; x <= Cells[Cells[current].Neighbour[1]].possible.Length - 1; x++)   //annyiszor futhat, amennyi
                            {
                                hasonlitas = Cells[Cells[current].Neighbour[1]].possible[x]; //fenti szomszédja
                            ...
                            int[] Neww = NewPossible.ToArray();
                            Cells[Cells[current].Neighbour[1]].possible = Neww;
                            Cells[Cells[current].Neighbour[1]].entropy = Neww.Length;  //entropyt beallitjuk
                        ...
                        if (Cells[current].Neighbour[2] < 70)
                        {
                            for (int x = 0; x <= Cells[Cells[current].Neighbour[2]].possible.Length - 1; x++)   //annyiszor futhat, amennyi
                            {
                                hasonlitas = Cells[Cells[current].Neighbour[2]].possible[x]; //fenti szomszédja
                                label1.Text = Convert.ToString(hasonlitas);
                                Thread.Sleep(0);
                                if (hasonlitas == 1)
                                {
                                    NewPossible.Add(1);
                            ...
                            label2.Text = Convert.ToString(Cells[Cells[current].Neighbour[2]].possible.Length - 1);
                            int[] Neww = NewPossible.ToArray();
                            Cells[Cells[current].Neighbour[2]].possible = Neww;
                            Cells[Cells[current].Neighbour[2]].entropy = Neww.Length;  //entropyt beallitjuk
                            ...
                            label3.Text = Convert.ToString(Cells[Cells[current].Neighbour[2]].possible[0]);


                        }
                        if (Cells[current].Neighbour[3] < 70)
                        {
                            for (int x = 0; x <= Cells[Cells[current].Neighbour[3]].possible.Length - 1; x++)   //annyiszor futhat, amennyi
                            {
                                hasonlitas = Cells[Cells[current].Neighbour[3]].possible[x]; //fenti szomszédja
                            ...
                            int[] Neww = NewPossible.ToArray();
                            Cells[Cells[current].Neighbour[3]].possible = Neww;
                            Cells[Cells[current].Neighbour[3]].entropy = Neww.Length;  //entropyt beallitjuk

                            Array.Clear(Neww, 0, Neww.Length); //clears array + list
                            NewPossible.Clear();

                            label4.Text = Convert.ToString(Cells[Cells[current].Neighbour[3]].entropy);
                            //entropyt beállítjuk
                    ...
                    /*
                               ...
                               0- felső 1-jobboldal 2-alul 3-baloldal 
            ...
            */
            ...
            if (SelectMode == 999) // felező
            {
                ...
                //test
                //label1.Text = Convert.ToString(SzamMagas);
                //label2.Text = Convert.ToString(SzamHossz);
                //label3.Text = Convert.ToString(ToDeleteR[0]);
                //label4.Text = Convert.ToString(InitialKezd);

                /*
                    while (Cells[oszlop].Bottom == false)
                    {
                ...
                label1.Text = Convert.ToString(SzamMagas);
                label2.Text = Convert.ToString(SzamHossz);
                label3.Text = Convert.ToString(Cells[0].Bottom);
                label4.Text = Convert.ToString(Cells[7].Right);
                */
                //Fontok = new Font("arial", FontStyle.Bold);
            ...
            else
            {
                ...
                if (SolutionSelectMode == 1)
                {
                    ...
                    //TOFIX
                    //Sometimes it crashes --> mert a Cells[Cells[SolutionCurrent].Neighbour[0]].SV nem nézi meg hogy a Cells[Cells[SolutionCurrent].Neighbour[0] néha 70, aka out of bounds.
                    ...
                    for (int i = 0; i < Steps.Count; i++)
                    {
                        for (int x = 0; x < Backtracking.Count; x++)
                        {
                            if (Steps[i] == Backtracking[x])
                            {
                                Steps.Remove(Backtracking[x]);
                            }
                        }
                    ...
                    */
                    ...
                    // Bejelüljük a steps lista elemeit megoldásként
                    /*
                     * 
                     *
                    string result = "";
            ...
            //AMI KÖZÖS A BRESULTBAN ÉS A RESULTBAN, AZT KIVESSZÜK ÉS AKKOR MŰKÖDIK

            /*
             * 

            while (SolutionCurrent != ExitPoint) { 
            //meghatározzuk hogy mennyi a PossiblePaths
            if (Cells[SolutionCurrent].Top == false)
            {
                PossiblePaths++;
                    SolutionTop = true;
            }

            if (Cells[SolutionCurrent].Right == false)
            {
                   PossiblePaths++;
                   SolutionRight = true;
            }

            if (Cells[SolutionCurrent].Bottom == false)
            {
                PossiblePaths++;
                    SolutionBottom = true;
            }

            if (Cells[SolutionCurrent].Left == false)
            {
                PossiblePaths++;
                    SolutionLeft = true;
            }
                ...
                if (PossiblePaths == 1)
                {
                    ...
                    if (SolutionTop == true)
                    {
                        SolutionCurrent = Cells[SolutionCurrent].Neighbour[0];
                        SolutionTop = false;
                    }

                    if (SolutionRight == true)
                    {
                        SolutionCurrent = Cells[SolutionCurrent].Neighbour[1];
                        SolutionRight = false;
                    }

                    if (SolutionBottom == true)
                    {
                        SolutionCurrent = Cells[SolutionCurrent].Neighbour[2];
                        SolutionBottom = false;
                    }

                    if (SolutionLeft == true)
                    {
                        SolutionCurrent = Cells[SolutionCurrent].Neighbour[3];
                        SolutionLeft = false;
                    }
            ...
            if(PossiblePaths > 1) { 
                    ...
                    SolutionCheckpoint = SolutionCurrent;

                        if (SolutionTop == true)
                            {
                                Sup = Cells[SolutionCurrent].Neighbour[0];
                            SolutionTop = false;
                            }

                        if (SolutionRight == true)
                        {
                            Sright = Cells[SolutionCurrent].Neighbour[1];
                            SolutionRight = false;
                        }

                        if (SolutionBottom == true)
                        {
                            Sdown = Cells[SolutionCurrent].Neighbour[2];
                            SolutionBottom = false;
                        }

                        if (SolutionLeft == true)
                        {
                            Sleft = Cells[SolutionCurrent].Neighbour[3];
                            SolutionLeft = false;
                        }



                        int[] SolutionSelectable = new int[4] { Sup, Sright, Sdown, Sleft };
                        int randomValue = 0;
                        while (randomValue <= 0)
                        {
                            randomValue = SolutionSelectable[random.Next(0, SolutionSelectable.Length)];
                        }
                        SolutionCurrent = randomValue;
                        Sup = 0;
                        Sright = 0;
                        Sdown = 0;
                        Sleft = 0;
                ...
                if (PossiblePaths == 0)
                {
                    SolutionCurrent = SolutionCheckpoint;
                }
            }
